...
contract InstallmentBondFactory is BondFactory {
    ...
    function issue(
        ...
        bytes memory data
    ) external override onlyOwner validYearOption(yearOption) returns(uint256 id) {
        require(activeDurationInDays < FACTOR, "Active duration should be shorter than 180 days");
        ...
        _minObligationTokenAmountPerBondList[id] = retrieveMinObligationList(tokenAmountPerBond, yearOption);

        emit Issued(id, bondQuantity, tokenAmountPerBond, rate);

    }

    function fulfillObligationsAndLock(uint256 id, uint256 tokenAmountPerBond) external onlyOwner {
        require(!(isDefaulted(id) || isDefaultedInTheory(id)), "Withdraw not allowed when defaulted");
        require((_numberOfTimesFulfilled[id]+1) * FACTOR != _bondMetadata[id].durationInDays, "All obligations fulfilled");
        require(
            tokenAmountPerBond >= _minObligationTokenAmountPerBondList[id][_numberOfTimesFulfilled[id]],
            "Minimum obligation not fulfilled"
        );
        require(
            ((tokenAmountPerBond - _minObligationTokenAmountPerBondList[id][_numberOfTimesFulfilled[id]])  * 1e18 / _minObligationTokenAmountPerBondList[id][_numberOfTimesFulfilled[id]]) <= 5e16 ,
            "Fulfillment should not exceed 5% above obligation"
        );

        uint256 tokenAmount = tokenAmountPerBond  * _bondMetadata[id].issuedQuantity;
    ...
    function withdraw(uint256 id, uint256 tokenAmount) external override onlyOwner {
        ...
        require(!(isDefaulted(id) || isDefaultedInTheory(id)), "Withdraw not allowed when defaulted");
        require(tokenAmount <= _designatedTokenPool[id] - _lockedTokenAmount[id], "Withdrawal token amount exceeds token amount in contract");
    ...
    function isDefaultedInTheory(uint256 id) public view override returns(bool) {
        bool cond1 = hasReachedMaturity(id) && _designatedTokenPool[id] < principalWithInterest(id);
        bool cond2 = !hasReachedMaturity(id) && (block.timestamp > (_numberOfTimesFulfilled[id] + 1) * 180 * 1 days);
    ...
    function lockedTokenAmountPerBond(uint256 id) public view returns(uint256) {
        return lockedTokenAmount(id) * 1e18 / _bondMetadata[id].issuedQuantity / 1e18;
    }

    function yearOptionToDays(uint256 option) public pure validYearOption(option) returns(uint256) {
    ...
    function yearOptionToObligationRateList(uint256 option) public view validYearOption(option) returns(uint256[] memory) {
        if (option == 0) {
            return THREE;
        } else if (option == 1) {
            return FIVE;
    ...
    function retrieveMinObligationList(uint256 tokenAmountPerBond,uint256 yearOption) public view validYearOption(yearOption) returns(uint256[] memory){
        uint256[] memory obligationRateList = yearOptionToObligationRateList(yearOption);
        uint256 length = obligationRateList.length;
        uint256[] memory obligationPerBondList = new uint256[](length);

        for(uint i = 0; i < length; i++) {
            obligationPerBondList[i] = tokenAmountPerBond * obligationRateList[i] / 100;
        }
