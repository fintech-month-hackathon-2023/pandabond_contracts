...
contract CallableBondFactory is BondFactory {
    ...
    event Called(uint256 indexed id, uint256 couponRate, uint256 couponRateOnCall);
    ...
    function call(uint256 id) external {
        ...
        require(timeElapsed(id) > _minObligationPeriod[id] * 1 days, "Minimum obligation period not reached");
        _couponRateOnCall[id] = _bondMetadata[id].couponRate *  timeElapsed(id) / (_bondMetadata[id].durationInDays * 1 days) ;
        _isCalled[id] = true;
    ...
    function issue(
        ...
        bytes memory data
            ...
            id = BondFactory(address(this)).issue(bondQuantity, minPurchasedQuantity,tokenAmountPerBond,ticker,durationDays,activeDurationInDays,rate,data);
            _minObligationPeriod[id] = durationDays / 2;
        }

    //getter

    function principalWithInterest(uint256 id) public override view returns(uint256) {
