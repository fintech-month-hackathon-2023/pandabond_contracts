...
contract DualCurrencyBondFactory is ERC1155Holder {
    ...
    function issue(
        ...
        uint256 tokenBAmountPerBond = (exchangeRate * tokenAAmountPerBond) /
            1e9;
        IDualCurrencyBond.BondData memory data = IDualCurrencyBond.BondData(
            tokenAAmountPerBond,
            tokenBAmountPerBond,
            block.timestamp,
        ...
        IDualCurrencyBond.BondMetadata memory metadata = IDualCurrencyBond
            .BondMetadata(
                ticker,
                address(_tokenA),
                address(_tokenB),
    ...
    function purchase(uint256 id, uint256 bondQuantity) external {
        ...
        uint256 tokenAmount = _bondToken
            .bondDataAsStruct(id)
            .tokenAAmountPerBond;
        ...
        _bondDB.incrementTVLByToken(tokenAmount, address(_tokenA));
    ...
    function withdraw(
        ...
        _designatedTokenAPool[id] -= tokenAmount;
        _bondDB.decrementTVLByToken(tokenAmount, address(_tokenA));
    ...
    function withdrawExcess(uint256 id) external onlyOwner {
        ...
        uint256 tokenAmount = _designatedTokenBPool[id];

        _designatedTokenBPool[id] = 0;
        _bondDB.decrementTVLByToken(tokenAmount, address(_tokenB));
    ...
    function redeem(uint256 id, uint256 bondQuantity) external {
        ...
        if (isCompleted(id)) {
            ...
            tokenAAmountPerBond = _bondToken
                .bondDataAsStruct(id)
                .tokenAAmountPerBond;
            state = 1;
        ...
        if (state == 0) {
            ...
            uint256 tokenAmount = bondQuantity *
                _bondToken.bondDataAsStruct(id).tokenAAmountPerBond;
            _designatedTokenAPool[id] -= tokenAmount;
            _bondDB.decrementTVLByToken(tokenAmount, address(_tokenA));
            _tokenA.transfer(msg.sender, tokenAmount);
        } else if (state == 2) {
            ...
            _bondDB.decrementTVLByToken(tokenAAmount, address(_tokenA));
            _bondDB.decrementTVLByToken(tokenBAmount, address(_tokenB));
    ...
    function deposit(uint256 id, uint256 tokenAmount) external onlyOwner {
        ...
        _designatedTokenBPool[id] += tokenAmount;
        _bondDB.incrementTVLByToken(tokenAmount, address(_tokenB));
    ...
    function markAsDefaulted(uint256 id) external {
        ...
        _bondDB.decrementTVLByToken(callerRewardA, address(_tokenA));
        _bondDB.decrementTVLByToken(callerRewardB, address(_tokenB));
    ...
    function markAsCompleted(uint256 id) external {
        ...
        _bondDB.decrementTVLByToken(toCaller, address(_tokenB));
    ...
    function couponRate(uint256 id) public view returns (uint256) {
        return _bondToken.bondDataAsStruct(id).couponRate;
    }

    function principal(uint256 id) public view returns (uint256) {
        return
            _bondToken.bondDataAsStruct(id).tokenBAmountPerBond *
            _purchasedQuantity[id];
    }
